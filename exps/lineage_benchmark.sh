...
for n_acc in "${load_account[@]}"; do
    for value_size in "${value_sizes[@]}"; do
        ...
        # ../build_release/bin/lineageBenchmarkV1 -a $n_acc -b $load_batch_size -t $num_transaction_version -z $num_transaction_account -l $query_versions -k $key_size -v $value_size -d $data_path -r $result_path
        ../build_release/bin/lineageBenchmarkV2 -a $n_acc -b $load_batch_size -t $num_transaction_version -z $num_transaction_account -l $query_versions -k $key_size -v $value_size -d $data_path -r $result_path
        sleep 5
        set +x
    done
