...
result_dir="$PWD/results_${db_name}/simple-payment-no-load_${test_name}"
log_dir="$PWD/logs/test_simple_payment_no_load_${test_name}"
...
seed=${seed}" | tee -a "$log_file"
    ...
    ${EXE} \
        ...
        -d ${data_path} \
        -i ${index_path} \
        -r ${result_path} 2>&1 | tee -a "$log_file"
