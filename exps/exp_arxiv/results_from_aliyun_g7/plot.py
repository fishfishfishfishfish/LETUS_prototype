...
detail_dir = f'{args.test_name}'
summary_file = f'{args.test_name}_results.csv'
summary_plot_file = f'{args.test_name}_results.png'

summary_dict = {"num acc":[], "batch_size":[], "value_size":[],
...
for entry in os.listdir(detail_dir):
    ...
    if os.path.isfile(full_path) and entry.endswith('.csv'):
        ...
        acc_bz, vl = fname.split('v')
        acc, bz = acc_bz.split('b')
        acc, bz, vl = int(acc.strip('acc')), int(bz.strip('b')), int(vl)
...
for acc, bz, vl, fn, fp in detail_files:
    ...
    plt.close()
    summary_dict["num acc"].append(acc)
...
plt.subplot(1, 2, 2)
plt.plot(df['num acc'].to_numpy()/1e6, df['get_latency'].to_numpy()*1000,
            marker='o', label=f'Get')
plt.plot(df['num acc'].to_numpy()/1e6, df['put_latency'].to_numpy()*1000,
            marker='o', label=f'Put')
plt.title('Latency (ms)')
plt.xlabel('Number of keys (M)')
plt.ylabel('Latency (s)')
plt.legend()
plt.grid(True)

plt.subplot(1, 2, 1)
plt.plot(df['num acc'].to_numpy()/1e6, df['get_throughput'].to_numpy()/1000,
            marker='o', label=f'Get')
plt.plot(df['num acc'].to_numpy()/1e6, df['put_throughput'].to_numpy()/1000,
            marker='o', label=f'Put')
plt.ylim(0,60)
...
plt.savefig("standalone2.png", dpi=300, bbox_inches='tight')
plt.close()
